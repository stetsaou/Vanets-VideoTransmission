...
/*void MonitorSnifferRxCallback(Ptr< const Packet > packet,
                              uint16_t channelFreqMhz,
...
std::cout<<"Packet received from " << source << " to " << dest << " on Chan " << channelFreqMhz << " with Signal " << signalDbm << " and noise " << signalNoise << std::endl;
...
NS_LOG_COMPONENT_DEFINE ("vanet-routing-compare-e");
  ...
  for (uint32_t i = 0; i < m_nSinks; i++)
    {
      ...
      //Inserindo o envio de videos entre os nodos

      EvalvidServerHelper server(m_port);
      server.SetAttribute ("SenderTraceFilename", StringValue("st_highway_cif.st"));
      server.SetAttribute ("SenderDumpFilename", StringValue("sd_a01_lte.st"));
      server.SetAttribute ("PacketPayload",UintegerValue(1014));
      ApplicationContainer apps = server.Install(c.Get (i + m_nSinks));
      apps.Start (Seconds (var->GetValue (1.0,2.0)));
      apps.Stop (Seconds (m_TotalSimTime));

      EvalvidClientHelper client (adhocTxInterfaces.GetAddress (i),m_port);
      client.SetAttribute ("ReceiverDumpFilename", StringValue("rd_a01_lte.st"));
      apps = client.Install (c.Get(i));
      apps.Start (Seconds (var->GetValue (1.0,2.0)));
      apps.Stop (Seconds (m_TotalSimTime));
...
VanetRoutingExperiment::VanetRoutingExperiment ()
  : m_port (9),
    ...
    // 1=802.11p
    m_80211mode (1),
    m_traceFile ("home/stelios/mobility.tcl"),
...
static ns3::GlobalValue g_logFile ("VRClogFile",
                                   "Log filename",
                                   ns3::StringValue ("vanetcompare2.filt.7.adj.log"),
                                   ns3::MakeStringChecker ());
...
VanetRoutingExperiment::Run ()
{
  ...
  AnimationInterface anim ("/home/stelios/sumo/tools/2022-05-25-16-02-29/trace.xml");
  AnimationInterface anim2 ("animation_experiment1-sumo.xml");
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator iter = stats.begin (); iter != stats.end (); ++iter)
  {
    ...
    //   NS_LOG_UNCOND("Rate (Mb/s): " <<iter->second. (iter->second.timeLastRxPacket.GetSeconds()-iter->second.timeFirstTxPacket.GetSeconds()) / 1024 /1024 << " Mbps");
...
/*       std::cout<<"Flow ID: " << iter->first << " Src Addr " << t.sourceAddress << " Dst Addr " << t.destinationAddress<<"\n";
      ...
      /* index   MCS  Rate (Mb/s) Tput (Mb/s) Received Signal (dBm)Noi+Inf(dBm) SNR (dB)
       ...
       flowmon->SerializeToXmlFile ("/home/stelios/sumo/tools/2022-05-25-16-02-29/trace.xml", true, true);
...
//  cmd.AddValue ("phyModeN", "Phy mode 802.11n; OfdmRate54Mbps or ", m_phyModeB);
...
VanetRoutingExperiment::ConfigureDefaults ()
{
  ...
  if (m_80211mode == 2)
    {
      Config::SetDefault ("ns3::WifiRemoteStationManager::NonUnicastMode",StringValue ("HtMcs1"));
      Config::SetDefault ("ns3::OnOffApplication::PacketSize",StringValue ("1472"));
    }
  else
    {
...
VanetRoutingExperiment::SetupAdhocDevices ()
{
  ...
  // valor corresponde a Molacao QPSK 1/2 Data Rate 13 (no SGI)  14.4 (SGI) em channel of 20MHz
  // em file:///home/doutorado/%C3%81rea%20de%20Trabalho/Experimentos/MCS-Index-802.11n-and-802.11ac.pdf
  oss << "HtMcs" << 1;
...
VanetRoutingExperiment::SetupScenario ()
{
  ...
  if (m_scenario == 1)
  ...
  else if (m_scenario == 2)
    {
      ...
      m_traceFile = "/home/stelios/ns-3-allinone/ns-3.29/src/wave/examples/low99-ct-unterstrass-1day.filt.7.adj.mob";
        m_logFile = "low99-ct-unterstrass-1day.filt.7.adj.log";
      m_mobility = 1;
      m_nNodes = 98;
